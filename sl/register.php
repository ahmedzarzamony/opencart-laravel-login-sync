...
class ControllerAccountRegister extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()  && $this->ahw_store_register()) {
	...
	public function callExternalApi($apiUrl, $postData) {
		$apiUrl = 'http://127.0.0.1:8000/' . $apiUrl;
		$ch = curl_init($apiUrl);
	...
	private function validate() {
		...
		if ($this->model_account_customer->getTotalCustomersByEmail($this->request->post['email'])) {
			...
			//==> AHW STORE
			if($e_check = $this->ahw_register_validate_email($this->request->post['email'])){
				if(@$e_check->exists){
					$this->error['warning'] = $this->language->get('error_exists');
				}
			}
